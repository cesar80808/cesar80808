# Hi, I'm César Flores! 👋

🎓 Computer Science student at the University of Guanajuato.

I am currently interested in competitive programming, algorithms, data structures, machine learning and mathematical modeling. 
I enjoy working on problems that combine theory with practical implementation, especially in C/C++ and Python.

## 🔬 Current Interests
- Competitive Programming
- Data Structures and Algorithms
- Machine Learning

## 💻 Technical Skills

### Languages
- C++
- Python
- Java
- SQL
- LaTeX

### Areas
- Advanced Data Structures
- Computational Geometry
- Optimization Algorithms
- Machine Learning
- Scientific Computing
- Parallel Computing
- Statistical Pattern Recognition

## 📚 Academic Projects

Some topics I have worked on include:
- To
- Be 
- Added...

## 👨‍🏫 Experience

**Teaching Assistant — Advanced Data Structures and Algorithms**

Graded programming assignments and algorithmic analyses for 29 fourth semester students covering advanced data structures, graph algorithms, and string processing.

**Teaching Assistant — Introduction to Computer Science**

Graded programming assignments for 37 first semester students on programming fundamentals, C programming, and algorithmic thinking.


## 🎨 Personal Interests
- Performing Arts
- Music lover
- Literature and Poetry
- Cinephile


## 📫 Let's Connect

- E-mail: <cesar.flores@cimat.mx>
- [LinkedIn](https://www.linkedin.com/in/cesar80808/)


>_Y la Muerte vencida se refugia temblando
>en el círculo estrecho del minuto presente._
>     Federico García Lorca. 
